...
function SettingsSheet({ open, onClose, state, updateProfile, exportData, importRef, importData, clearSubstitutions }) {
  const { profile, exerciseSubstitutions } = state;
  return (
    <AnimatePresence>
      {open ? (
        <motion.div className="motor-settings-layer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
          <motion.section className="motor-settings-sheet" role="dialog" aria-modal="true" aria-labelledby="motor-settings-title" initial={{ y: 70, opacity: 0, scale: .97 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 48, opacity: 0, scale: .98 }} transition={{ type: "spring", stiffness: 420, damping: 34 }}>
            ...
            <div className="motor-settings-head"><div><span>MOTOR 4.2</span><h2 id="motor-settings-title">Configuración</h2></div><motion.button type="button" whileTap={{ scale: .9 }} onClick={onClose}>Listo</motion.button></div>
            <div className="motor-settings-grid">
              <Field label="OBJETIVO"><select className="control" value={profile.objetivo} onChange={updateProfile("objetivo")}>{Object.entries(PROGRAM_GOAL_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field>
              {profile.objetivo === "pr" ? <Field label="LEVANTAMIENTO PR"><select className="control" value={profile.prLift} onChange={updateProfile("prLift")}>{PR_LIFT_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field> : null}
              <Field label="SPLIT / FRECUENCIA"><select className="control" value={profile.dias} onChange={updateProfile("dias")}>{SPLIT_OPTIONS.map(([days, label]) => <option key={days} value={days}>{days} días · {label}</option>)}</select></Field>
              ...
              <Field label="DURACIÓN"><select className="control" value={profile.duracion} onChange={updateProfile("duracion")}>{[45,60,75,90].map((value) => <option key={value} value={value}>{value} min</option>)}</select></Field>
            ...
            <div className="data-actions motor-data-actions">
              <button className="secondary-btn" type="button" onClick={clearSubstitutions} disabled={!Object.keys(exerciseSubstitutions).length}>Restablecer ejercicios</button>
...
export default function AppV42() {
  ...
  const updateProfile = (key) => (event) => {
    if (activeWorkout) {
      setNotice("Finaliza la sesión activa antes de cambiar la programación.");
      return;
    }
  ...
  const updateReadiness = (key) => (event) => setState((current) => ({ ...current, readiness: { ...current.readiness, [key]: Number(event.target.value) } }));
  ...
  const startWorkout = () => {
    ...
    const workout = createWorkoutSession(session, activeDay, uid);
    setState((current) => ({ ...current, activeWorkout: workout }));
    setNotice(`Sesión iniciada · ${session.label}.`);
  ...
  const changeDay = (index) => {
    if (activeWorkout && activeWorkout.sessionIndex !== index) {
      setNotice(`Finaliza ${activeWorkout.sessionLabel} antes de cambiar de día.`);
      return;
    }
    setActiveDay(index);
  };
      ...
      <div className="container">
        <header className="header v4-hero">
          <div className="v4-hero-copy">
            <p className="eyebrow">MOTOR FITNESS 4.2</p>
            <div className="v4-status-row"><span>{PROGRAM_GOAL_LABELS[profile.objetivo]}</span><span>{EXPERIENCE_LABELS[profile.experiencia]}</span><span>Semana {plan.mesocycle.week}/6</span></div>
            <h1>{session.label}</h1>
            <p className="subtitle">Sesiones persistentes, sustituciones inteligentes y autorregulación por rendimiento repetido.</p>
            <div className="v4-hero-actions">
              <motion.button className="primary-btn" type="button" whileTap={{ scale: .96 }} onClick={startWorkout}>{activeWorkout ? `Sesión activa · ${formatDuration(activeElapsed)}` : "Iniciar entrenamiento"}</motion.button>
        ...
        <section className="card session-overview">
          ...
          <div className="session-overview-row">
            <div><span>Estado</span><strong>{activeWorkout ? `Activa · ${formatDuration(activeElapsed)}` : "Sin iniciar"}</strong></div>
            <div><span>RIR real</span><strong>{plan.programming?.targetRir ?? plan.mesocycle.targetRir}</strong></div>
            <div><span>Fatiga</span><strong>{performanceFatigue.label}</strong></div>
          </div>
        </section>
        ...
        <div className="v4-main-grid">
          <section className="stack">
            <section className="card routine-card">
              ...
              <div className="session-head"><div><h2>{session.label}</h2><span>{activeWorkout ? `En curso · mínimo ${progress.minimumToFinish} ejercicios para cerrar` : "Inicia para registrar esta sesión como entidad real"}</span></div><button className="secondary-btn" type="button" onClick={finishWorkout} disabled={!activeWorkout}>Finalizar</button></div>
                ...
                {session.exercises.map((exerciseItem, exerciseIndex) => {
                  ...
                  return (
                    <motion.article className="exercise" key={`${exerciseItem.substitutionSourceId || exerciseItem.id}-${exerciseItem.id}`} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: .32, delay: Math.min(exerciseIndex * .035, .18) }}>
                      ...
                      {!isLockedPr && options.length > 1 ? (
                        <div className="warmup"><span>Sustitución</span><select className="control" value={exerciseItem.id} disabled={Boolean(activeWorkout)} onChange={(event) => setSubstitution(session, exerciseItem, event.target.value)}>{options.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select></div>
                      ) : null}
            ...
            <section className="card recovery-card">
              <div className="section-heading"><h2 className="section-title">AUTORREGULACIÓN</h2><span>{plan.readiness.score}/15 subjetivo</span></div>
              <div className="readiness-grid">{[["energia","ENERGÍA","1 baja · 5 alta"],["sueno","SUEÑO","1 malo · 5 bueno"],["dolor","AGUJETAS","1 bajas · 5 altas"]].map(([key,label,hint]) => <Field key={key} label={label} hint={hint}><div className="range-line"><input aria-label={label} type="range" min="1" max="5" value={readiness[key]} onChange={updateReadiness(key)} /><span>{readiness[key]}</span></div></Field>)}</div>
              <div className={`readiness-status status-${performanceFatigue.status}`}><div><span>Subjetivo</span><strong>{plan.readiness.label}</strong></div><div><span>Objetivo</span><strong>{performanceFatigue.label}</strong></div><div><span>RIR</span><strong>{plan.programming?.targetRir}</strong></div></div>
              <p className="body-copy">{performanceFatigue.message}</p>
            </section>
          </section>
          ...
          <aside className="v4-side-stack">
            ...
            <details className="card program-details"><summary><span>PROGRAMACIÓN</span><strong>{plan.programming.title}</strong></summary><p className="body-copy">{plan.programming.summary}</p><div className="program-mini-grid"><div><span>Principales</span><strong>{plan.programming.primaryRange}</strong></div><div><span>Accesorios</span><strong>{plan.programming.accessoryRange}</strong></div><div><span>Descanso</span><strong>{plan.programming.rest}</strong></div></div><p className="body-copy"><strong>Split:</strong> {plan.programming.splitNote}</p>{plan.programming.warning ? <div className="empty compact">{plan.programming.warning}</div> : null}</details>

            <section className="card volume-card"><div className="section-heading"><h2 className="section-title">VOLUMEN SEMANAL</h2><span>series eq.</span></div><div className="volume-grid">{Object.entries(plan.weeklyVolume).filter(([, value]) => value > 0).map(([muscle, value]) => <div className="volume-chip" key={muscle}><span>{muscle}</span><strong>{value}</strong></div>)}</div></section>
          </aside>
        </div>
